			...
			success : function(result){
				...
				/* for(let j = 0; j < trainerUserManagement.length; j++) {
					for(let i = 0; i < commentList.length; i++) {
						if(trainerUserManagement[j].classId === commentList[i].classId) {
							trainerUserManagement[j] = commentList[i];
						}												
					}
				 }
				...
				$(trainerUserManagement).each(function() {
					...
					var userIndex = -1;
					if(list2[i].classId == this.classId) {
							userIndex = i
						}
					...
					for(var i = 0; i < commentList.length; i++){
						if(commentList[i].classId == this.classId) {
							index = i;
							for(var j=0; j<commentList[i].iuserList.length; j++) {
								if(list2.)
								htmls += '<p class="pt_reply_content">'+ this.classId + ':' + commentList[i].iuserList[j].icommentList[0].commentContent+'</p>';
							}
						}
					}					
					if (index < 0){

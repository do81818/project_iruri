	...
	<div class="wrap">
		...
		<div id="admin_memberTabMenu">
			<input type="radio" id="tap1" name="taps"
				onclick="getlist(1,'all',0,0)" value="1" checked> <label
				for="tap1">전체수익</label> <input type="radio" id="tap2" name="taps"
				onclick="loadList('trainer')" value="2"> <label for="tap2">트레이너별
				수익</label>
		</div>
					...
					<table>
						...
						<tr>
							...
							<td class="inquiry_quick_button">
								<button type="button" onclick="simpleInquire('lately')">최근 30건</button>
		...
		<script type="text/javascript">
			function valueCheck() {
				...
				if ((sy != null && sy != "") && (sm != null && sm != "")
					...
					if ((periodStartDate < todayStr && periodEndDate <= periodEndDate)
						...
						getlist(1, inquire, periodStartDate, periodEndDate);
			...
			function getlist(page, inquire, periodStartDate, periodEndDate) {
				$.ajax({
					...
					success : function(result) {
						...
						if (list.length < 1) {
							...
							$(list).each(function() {
								htmls += '<tr>'
									+ '<td class="table_No_date">' + moment(this.moneyVo.moneyDate).format('YYYY.MM.DD') + '</td>'
									+ '<td class="table_indigo_text">' + this.iclassVo.classTitle + '</td>'
									+ '<td class="table_indigo_text">' + this.iuserVo.userNickname + '</td>'
									+ '<td class="table_blue_text">' + priceToString(this.moneyVo.moneyInput) + '</td>'
									+ '<td class="table_red_text">'+ priceToString(this.moneyVo.moneyOutput) + '</td>'
									+ '<td class="table_blue_text">' + priceToString(this.totalMoney) + '</td>'
									+ '</tr>';
							...
							if (pagination['prev']) {
								htmls2 += '<a class="arrow prev" href="#admin_memberTabMenu" onclick="javascript:getlist(\''
										+ (pagination['startPage'] - 1) +'\',\''+ inquire + '\',\'' + periodStartDate + '\',\''+ periodEndDate
										+ '\');"></a>';
							}
							...
							for (var idx = pagination['startPage']; idx <= pagination['endPage']; idx++) {
								if (page !== idx) {
									htmls2 += '<a class="pageNumLink" href="#admin_memberTabMenu" onclick="javascript:getlist(\''
											+ idx  +'\',\''+ inquire + '\',\'' + periodStartDate + '\',\'' + periodEndDate + '\');">' + (idx) + "</a>";
								} else {
							...
							if (pagination['next']) {
								htmls2 += '<a class="arrow next" href="#admin_memberTabMenu" onclick="javascript:getlist(\''
										+ (pagination['endPage'] + 1) + '\',\'' + inquire + '\',\'' + periodStartDate + '\',\'' + periodEndDate
										+ '\)\"></a>';
			...
			function simpleInquire(type) {
				...
				if(type == 'lately'){
					...
					let periodEndDate = String(today.getFullYear())
								+ dateCheck((today.getMonth() + 1))
								+ dateCheck(today.getDate());
					getlist(1, inquireType, periodStartDate, periodEndDate);
			...
			/* document.getElementById("inquiry_conditions_30").addEventListener("click", function() {
						simpleInquire('30');
					}, false); */

						...
						<div class="container content">
								...
								<div class="page_nation">
										<c:if test="${pageMaker.prev}">
												<a class="prev"
														href="pointlist${pageMaker.makeQuery(pageMaker.startPage - 1) }"></a>
										</c:if>
												...
												<c:choose>
														<c:when test="${pageMaker.cri.pageNum == num}">
																<a class="pageNumLink active"
																		href="pointlist${pageMaker.makeQuery(num)}">${num}</a>
														</c:when>
														<c:otherwise>
																<a class="pageNumLink"
																		href="pointlist${pageMaker.makeQuery(num)}">${num}</a>
										...
										<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
												<a class="next"
														href="pointlist${pageMaker.makeQuery(pageMaker.endPage +1) }"></a>
										</c:if>
								</div>

								<form id="actionForm" action="mypage_user/mypage_user_pointlist"
										method="get">

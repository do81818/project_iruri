...
${sumList3[status.index].데이터명} --%>
								...
								<c:forEach var="confirm" items="${commentlist}"
										varStatus="status">
										...
										<c:if test="${confirm.categoryId == 5}">
												<div class="m_userboardlist">
														...
														<div id="m_board_list_img">
																<!-- <img id="img"
																		src="/ex/resources/src/img/icon/360-250.png"> -->
														<img id="img" alt=""  src="${CONTEXT_PATH}/iruri/display?fileName=${confirm.boardFile}" alt="" > 
														
														</div>
														<div id="m_board_list_content">
																<div id="m_board_list_title">
																		<a href="/ex/iruri/challenge_detail_after?classId=${confirm.boardGroupId}">
																				<c:out value="${confirm.boardTitle}" />
																		</a>
																</div>
																<div id="m_board_list_content">
																		<c:out value="${confirm.boardContent}" />
																</div>
																<div id="m_board_list_date">
																		<%@ taglib prefix="fmt"
																				uri="http://java.sun.com/jsp/jstl/fmt"%>
																		<fmt:formatDate pattern="yyyy-MM-dd"
																				value="${confirm.boardDate}" />
																</div>
												...
												<div class="m_userboardlist">
														...
														<div id="m_board_list_content">
																<div id="m_board_list_content">
																		...
																		<a
																				href="/ex/iruri/challenge_detail_after?classId=${confirm.boardGroupId}">
																				<c:out value=" ${confirm.boardContent}" />
																		</a>
																</div>
																<div id="m_board_list_date">
 ...
 -->
								...
								<div class="page_nation">
										<c:if test="${pageMaker.prev}">
												<a class="prev"
														href="boardlist${pageMaker.makeQuery(pageMaker.startPage - 1) }"></a>
										</c:if>
										...
										<c:forEach var="num" begin="${pageMaker.startPage}"
												...
												<c:choose>
														<c:when test="${pageMaker.cri.pageNum == num}">
																<a class="pageNumLink active"
																		href="boardlist${pageMaker.makeQuery(num)}">${num}</a>
														</c:when>
														<c:otherwise>
																<a class="pageNumLink"
																		href="boardlist${pageMaker.makeQuery(num)}">${num}</a>
										...
										<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
												<a class="next"
														href="boardlist${pageMaker.makeQuery(pageMaker.endPage +1) }"></a>
										</c:if>
								</div>

								<form id="actionForm" action="mypage_user/mypage_user_boardlist"
										method="get">
												...
												<div class="alertWindow_insert">
														...
														<form action="update" method="GET">
																...
																<input class="inputbox_size1" name="userPw" type="text"
																		placeholder="8~16자리의 영문 대소문자, 숫자 및 특수문자 사용"
																		autocomplete="off" />
																...
																<p>전화번호</p>
																<input class="inputbox_size2" name="userPhone"
																		type="text" autocomplete="off">
